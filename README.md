...
MCP 的连接、工具参数和离线写入说明见 [MCP 使用说明](docs/mcp.md)。本机客户端可创建访问令牌，通过 Streamable HTTP 或 stdio 连接；ChatGPT 官方客户端可运行 `pnpm chatgpt` 启动临时 HTTPS 入口，在连接设置准备手账并确认 OAuth 授权。用户登录与正式部署仍留到后续。
