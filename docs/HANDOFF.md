...
- 2026-09-10 最新授权：实现 MCP 各项工具。本机 MCP 的 7 项工具、真实手账令牌、离线 Note 写入与网页接收已完成，见 [MCP 说明](mcp.md)。用户已自行确认分享导入、手机体验、另一设备模型正常返回，压缩质量尚未验收。账号仍为最后阶段；OAuth、公网部署与跨设备同步尚未接入。本地模型配置保持 Git 忽略。
...
- 真实摘要和本地后台执行已实现，用户确认另一设备模型正常返回，摘要质量仍待验收。本机 MCP 已实现；尚无公网 MCP、OAuth、云端账号、自动文件同步或向量检索。回收站仍在页面启动、聚焦和每小时执行清理；任务进程需保持运行，电脑休眠时不执行。连接页的 MCP 令牌已真实生效，官方 OAuth 标为待接入。
