...
- 2026-09-11：已拉取 main 的 `ea69c60`，审查 ChatGPT OAuth 并按同一流程补充 Claude 远程连接器。用户补充要求未来可添加其他 OAuth 客户端；已统一注册表、回调策略、客户端文案和界面选项，协议流程不按厂商分支。Claude 官方客户端实际联调待用户执行；本轮不发布公网入口或操作真实手账测试。
...
- 当前下一步为真实压缩质量、备份恢复界面与大量附件存储成本验收；分享导入、手机体验、模型正常返回已有用户确认。本机 MCP 已完成，账号与公网连接为后续阶段；平台来源限制保留为运行环境差异，不重复列为当前验收阻塞。
