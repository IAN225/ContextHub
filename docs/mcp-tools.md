...
客户端压缩由模型提交的覆盖范围决定原文保留：所有未被活跃客户端摘要覆盖的正常轮次都会保留，包括后续新增轮次；不再使用客户端轨道原有的固定轮数或 token 窗口。页面只显示保留轮数。`recent_from_turn` 表示最早的保留轮次，不是必须压缩到的边界；模型自行选择 `conversation_read` 的范围。自定义压缩和 ReMeLight 的窗口设置保持独立。记忆包中显式自定义的原文块仍可单独限制输出轮数。
