...
分享导入需要部署服务器能访问来源站点。公开页面可在浏览器打开，并不保证来源的数据或图片接口允许无登录的服务器请求。Claude 限制访问时请复制消息手动导入。ChatGPT 同一图片的正文引用与元数据只导入一次；公开文件接口可用时获取下载地址并交由附件任务保存，否则保留具体失败原因，可在原文编辑中补充原图。
