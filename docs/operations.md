...
在部署域名检查 320、390、600、760 像素宽度：返回、工作区设置（齿轮）、全局设置（滑杆）和其他操作保持在顶部同一行；原文、摘要、Note、记忆包、连接保持五列一行。480 像素及以下的“收录对话”显示为加号，长工作区名称省略显示。打开工作区设置后齿轮高亮，其他章节仍可切换；全局设置仍在保存完成后才允许跳转。
