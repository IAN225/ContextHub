...
客户端投递和手动导入的正文上限为 **8 MiB**，按 UTF-8 字节计算；图片与附件字节不计入正文额度。单次请求和整理后的对话记录各不超过 **32 MiB**，包含附件、JSON 结构及轮次信息。单个附件仍为 **5 MiB**，每次最多 **12,000 条消息**（一轮通常包含多条消息）。

账号待处理的客户端收件最多 **200 份、合计 64 MiB**，达到任一上限后需先归档或删除收件。账号总存储额度仍为 **512 MiB**。这些是输入和存储限制，不代表已验证的并发或性能容量。

## 网页按工作区备份与导入

在“数据管理”勾选一个或多个工作区，再点击“导出所选工作区”。例如账号有 5 个工作区，可以只导出其中 3 个；新环境选择该 JSON 文件后，会列出文件中的 3 个工作区，可以只勾选其中 1 个导入。

- 工作区备份包含所选工作区的原文、已保存附件、Note 与历史、全部摘要轨道、记忆包配置、对应草稿、关联待归档内容及 Note 通知。不包含其他工作区、未归属收件、账号级偏好、凭据或接收回执。
- 默认追加导入为独立工作区。导入时重新分配工作区与收件 ID，保留工作区内部的轮次、摘要覆盖、Note 与附件引用；同名或同 ID 的现有工作区不会被覆盖。现有连接、后台任务、收件和偏好不重置。
- 导入副本的模型配置标记与摘要自动运行关闭，需要重新配置模型和 MCP。导入回收站项目重新获得 30 天保留期，不延长当前账号其他回收站内容的期限。
- 仍可选择“导出完整账号备份”，包含账号偏好及全部收件。高级“完整覆盖恢复”沿用整个备份覆盖当前账号的行为，不使用工作区勾选；需要明确确认，并在覆盖前下载当前数据备份。该模式会清除旧任务与连接。
- JSON 格式保持 v2，旧 v1/v2 文件均可读取后选择工作区。服务端完整实例备份与升级回滚流程不受影响。
...
历史 MCP 快照、后台任务状态和任务结果使用带版本的数据封套，传输类型按版本固定在 `app/lib/storage/payload-v1.ts`、`payload-v2.ts`、`payload-v3.ts`，与页面的 Workspace 类型分开。升级可读取原先没有封套的记录，未知新版本会被拒绝处理。后续发布若改变字段语义，应新增版本和转换器，并验证旧的排队任务与未接收结果；不以清空数据库代替迁移。
...
原文轮次不再保存标题，按工作区内的编号定位。旧数据库、收件、账号备份和编辑草稿读取时移除该字段，消息、附件、轮次 ID 与摘要覆盖关系保留。原文和收件记录写入格式 v2；新账号导出备份为 v2，仍可导入 v1。账号 SQL schema 不变。

后台任务使用独立的 payload v3，继续读取旧任务快照。升级前已经生成但尚未应用的摘要，如果版本凭据与当前原文不一致，会进入候选收件供确认，不会直接覆盖活跃摘要。升级后浏览器协议为 6，旧页面须刷新。降级请恢复升级前完整备份，不要直接用旧服务打开新记录。
