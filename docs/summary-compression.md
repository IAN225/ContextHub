...
5. 点击检查点列表底部固定的“新建自定义摘要”，在右侧组合历史摘要和完整原文、填写重点要求。草稿自动保存，生成候选后单独确认，再选择应用时的水位策略。切回检查点可继续预览已有摘要。

摘要提示词和记忆包共同使用 `use-block-reorder.ts` 的指针排序逻辑，包含实时补位、占位框、浮动预览和 Escape 取消；上下移动按钮继续支持键盘操作。

## 压缩与保存规则

- 顶部参数与压缩按钮同行，渐变竖线分隔操作：`保留近期 [数值] [token/轮] 原文`、`每批发送上限 [数值] [token/轮]`。单位下拉框无边框和底色，浅色提示按当前窗口或下一批显示约合轮数/token；窄屏自然换行。
- 每批上限支持轮数和 token，旧数据默认轮数；token 上限对完整请求（提示词、已有摘要、原文）保守计数，同时受模型上下文与输出预留约束。第一整轮仍放不下时暂停，不截断、不跳过；切换单位保留各自数值，配置随备份与后台任务快照保存。
- 原文保留窗口可选择“按轮数”或“按 token”，旧数据默认按轮数。窗口仍从处理水位后的第一轮开始，依次纳入完整轮次；若下一轮累计超过 token 上限，就将整轮及后续轮次留在窗口外，不截断、不跳过。刚好等于上限时纳入；第一轮就超过时窗口为空。
- 压缩停止位置由原文末尾能完整放入同一上限的连续轮次决定；末尾单轮也放不下时，它仍是待压缩原文。token 采用 UTF-8 字节与消息开销的保守估算，包含工具消息和已提取附件文字，不计二进制文件字节，也不使用来源的整次请求 usage 冒充单轮长度。不是特定模型 tokenizer 的精确数值。
- 动态记忆包窗口跟随相同设置；自选记忆包的轮数仍独立生效。窗口设置随备份和后台快照保存，改变设置后旧摘要结果不会覆盖新水位。
