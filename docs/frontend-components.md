...
导航 hook 始终跟随章节挂载，即使筛选没有结果也不卸载。Preview/Payload/元数据选项和 Markdown 开关由章节持有，详情组件受控，避免切换轮次、暂时无结果或隐藏章节后重置。滚轮和键盘同步更新目标引用，触屏才从滚动位置更新选择；104px 刻度与原来的手势处理保留。样式仍归 `transcript.css`。
...
拆分不新增 DOM 包装、CSS 加载入口、业务命令或存储键。后续修改先定位上表中的职责，再改相应样式；跨章节规则继续放领域层。不要仅为了减少单个文件行数继续切碎同一业务。
