...
三种方案共享原文、附件和 Note，独立保存最多 30 个历史检查点、活跃摘要和处理位置。客户端压缩不需要配置摘要模型，也不创建后台模型任务。
...
2. 调用 `summary_read`，参数 `{"engine":"client"}`。返回 `summary`（旧活跃摘要或 `null`）、`revision` 和 `recent_from_turn`（最早保留轮次或 `null`，不是压缩终点）。
...
启用客户端方案的工作区记录使用 v2 格式，其他工作区继续使用 v1；旧程序会拒绝读取 v2 工作区，避免降级写入丢失新增方案。网页协议为 6，升级后旧网页需刷新。本次不改变 SQL 表结构，账号数据库仍为 schema 5。使用仓库部署脚本完成备份、兼容性检查和失败恢复，升级后不要直接切回不支持客户端摘要的旧程序；需要回退时应使用对应的升级前备份。后台任务当前写入 v3，继续读取 v1/v2；旧任务中已移除的轮次标题在读取时转换，过期结果仍通过候选收件确认。
