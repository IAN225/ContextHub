...
注册审批、随机初始密码、强制激活与角色管理以 [部署与激活指南](deployment-and-activation.md) 为准。下文保留账号数据仓库设计及上一版验收记录。
...
- 已支持审批制注册、关闭注册和管理员角色调整；服务器命令仍可创建账号或重置密码。
...
- 当前不提供附件对象存储、邮箱找回或实时多人协作。注册采用管理员审批制。
